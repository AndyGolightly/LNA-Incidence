## Run LNA inference code (FFMH)

source("helper.r")
source("inferenceLNA.r")

# Example: data set D_2

# Read in data
obs2 = scan("obs2.dat") 

# Tuning matrix for random walk proposal
sigtuneLNA = matrix(c(0.038128127, 0.03665185, -0.003598614,
0.036651851, 0.06166513, 0.018442814,
-0.003598614, 0.01844281, 0.028652206), ncol=3)

iters = 5000 # no. iterations
x0 = c(359,1) # initial state (S0,I0)
n0 = c(0,0) # initial incidence mean
V0 = diag(c(0,0)) # initial incidence variance
a = c(10,10,0) # prior hyper-parameters
b = c(10^4,30,1)
init1 = c(exp(-7),3*exp(-2.5),0.8) #ground truth
inter = 10 # inter-observation time
ODEdt = 0.1 # Euler time step for LNA ODEs
Fobs = c(1,0) # pick out observed component -- infections in this case

# Run MH scheme (posterior output on log scale)
set.seed(1)
system.time(outLNA<-LNAinf(iters,1.3*sigtuneLNA,x0,obs2,n0,V0,a,b,init1,alpha,beta,Fmat,Sepi,inter,ODEdt,Fobs))


 
